...
def merge_box(box0, box1):
	x0 = min(box0.bd[0], box1.bd[0])
	x1 = max(box0.bd[1], box1.bd[1])
	y0 = min(box0.bd[2], box1.bd[2])
	y1 = max(box0.bd[3], box1.bd[3])
...
def cal_Homo_2l_homo(p0, p1): # from p0 to p1
	A = np.zeros((2,9))
...
def cal_Homography_Linear_Least_Squares(corr_points): #from corr_points[0] to corr_points[1]
	# construct left-hand side
	for i in range(len(corr_points)):
		pt0 = corr_points[i][0]
		pt1 = corr_points[i][1]
		if (i == 0):
			A = cal_Homo_2l_homo(pt0, pt1)
		else:
			A_temp = cal_Homo_2l_homo(pt0, pt1)
			A = np.concatenate((A,A_temp), axis = 0)

	# find linear least squares
	# do svd
	u, s, vh = np.linalg.svd(A)
	# take the eigenvector of v corresponding to the smallest eigenvalue of s
	h = vh[len(vh)-1]
