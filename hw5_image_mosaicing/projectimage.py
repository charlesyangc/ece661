...
def mosaic_image_out2in_individual(img, H, box, shrink_coeff, imgc):
	size = img.shape
	lenxc = box.bd[1] - box.bd[0] + 1 #intermediate image
	lenyc = box.bd[3] - box.bd[2] + 1 #intermediate image

	lenx = shrink_coeff * lenxc # output image
	leny = shrink_coeff * lenyc # output image

	xc0 = box.bd[0]
	yc0 = box.bd[2]

	print('mosaic_image_out2in_individual:, xc0, yc0', xc0, yc0)
...
def mosaic_image_out2in(imgs, Hs2center):
	# first find the boundary of mosaiced image:
	box = find_img_project_boundary(imgs[0], Hs2center[0])
	for i in range(5):
		box_temp = find_img_project_boundary(imgs[i], Hs2center[i])
		print('Hs2center[i]:,', Hs2center[i])
		print('box.box_temp:, ',box_temp.bd)
		box = merge_box(box, box_temp)

	shrink_coeff = 1

	# next prepare the canvas:
	lenxc = box.bd[1] - box.bd[0] + 1 #intermediate image
	lenyc = box.bd[3] - box.bd[2] + 1 #intermediate image
	...
	for i in range(len(Hs2center)):
		mosaic_image_out2in_individual(imgs[i], Hs2center[i], box, shrink_coeff, imgc)
